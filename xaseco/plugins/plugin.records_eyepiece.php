 ...
 * http://labs.undef.de/XAseco1/Records-Eyepiece.php
 *
 * ----------------------------------------------------------------------------------
 * Author:		undef.de
 * Contributors:	.anDy
 ...
 *  - plugins/plugin.gerymania.php		Optional, only if you want the Gerymania-Widget (only for Gamemode 'Stunts'. Get it here: http://www.tm-forum.com/viewtopic.php?f=127&t=27398)
 ...
 * 91849 - 91898		UNUSED BUT RESERVED
...
function re_onSync ($aseco, $reload = null) {
	...
	if ( defined('XASECO_VERSION') ) {
		if ( version_compare(XASECO_VERSION, $xaseco_min_version, '<') ) {
			trigger_error('[plugin.records_eyepiece.php] Not supported XAseco version ('. XASECO_VERSION .')! Please update to min. version '. $xaseco_min_version .'!', E_USER_ERROR);
		}
	}
	else {
	...
	$scorelists = array('TOP_AVERAGE_TIMES', 'DEDIMANIA_RECORDS', 'GERYMANIA_RECORDS', 'LOCAL_RECORDS', 'TOP_RANKINGS', 'TOP_WINNERS', 'MOST_RECORDS', 'MOST_FINISHED', 'TOP_PLAYTIME', 'TOP_DONATORS', 'TOP_NATIONS', 'TOP_TRACKS', 'TOP_VOTERS', 'TOP_VISITORS', 'TOP_ACTIVE_PLAYERS', 'TOP_WINNING_PAYOUTS', 'TOP_BETWINS', 'TOP_ROUNDSCORE');
	...
	foreach ($widgets as &$widget) {
		foreach (range(0, 5) as $gamemode) {
			if ( ($widget == 'DEDIMANIA_RECORDS') && ($re_config[$widget][0]['GAMEMODE'][0][$gamemode][0]['ENABLED'][0] == true) ) {
			...
			if ( ($widget == 'GERYMANIA_RECORDS') && ($re_config[$widget][0]['GAMEMODE'][0][$gamemode][0]['ENABLED'][0] == true) ) {
			...
			if ( ($widget == 'LOCAL_RECORDS') && ($re_config[$widget][0]['GAMEMODE'][0][$gamemode][0]['ENABLED'][0] == true) ) {
			...
			if ( ($widget == 'LIVE_RANKINGS') && ($re_config[$widget][0]['GAMEMODE'][0][$gamemode][0]['ENABLED'][0] == true) ) {
	...
	if ($reload === null) {
		$aseco->console('***************[plugin.records_eyepiece.php]***************');
		...
		if ($res) {
			if (mysql_num_rows($res) > 0) {
				...
				while ($row = mysql_fetch_object($res)) {
					$query2 = "UPDATE `players_extra` SET `mostfinished`='". $row->Count ."' WHERE `playerID`='". $row->playerID ."';";
					$result = mysql_query($query2);
					if (!$result) {
						$aseco->console('   + Could not UPDATE `mostfinished` for playerID "'. $row->playerID .'" [for statement "'. $query2 .'"]!');
					}
				}
				$aseco->console('   + '. $entries .' '. (($entries == 1) ? 'entry' : 'entries') .' updated.');
			}
		...
		if ($res) {
			if (mysql_num_rows($res) > 0) {
				$entries = mysql_num_rows($res);
				while ($row = mysql_fetch_object($res)) {
					$query2 = "UPDATE `players_extra` SET `mostrecords`='". $row->Count ."' WHERE `playerID`='". $row->PlayerId ."';";
					$result = mysql_query($query2);
					if (!$result) {
						$aseco->console('   + Could not UPDATE `mostrecords` for playerID "'. $row->PlayerId .'" [for statement "'. $query2 .'"]!');
					}
				}
				$aseco->console('   + '. $entries .' '. (($entries == 1) ? 'entry' : 'entries') .' updated.');
			}
...
function re_onPlayerConnect2 ($aseco, $player) {
	...
	if ( ($re_config['JOIN_LEAVE_INFO'][0]['ENABLED'][0] == true) && ($aseco->startup_phase == false) ) {
		...
		$nation = $re_config['IocNations'][mapCountry($player->nation)];
...
function re_onGerymaniaRecord ($aseco, $record) {
	...
	// Player reached an new Record at the Challenge, need to Update the 'GerymaniaRecords'
	$re_config['States']['GerymaniaRecords']['NeedUpdate']		= true;
	$re_config['States']['GerymaniaRecords']['UpdateDisplay']	= true;
}
...
function re_onJukeboxChanged ($aseco, $command) {
	...
	if ($command[0] == 'clear') {
		...
		// Rebuild the Widgets
		$re_cache['ChallengeWidget']['Window']	= re_buildChallengeLastCurrentNextWindow();
...
function re_onNewChallenge2 ($aseco, $challenge_item) {
	...
	$re_config['Challenge']['Current']['env']			= $aseco->server->challenge->environment;
	$re_config['Challenge']['Current']['mood']			= $aseco->server->challenge->mood;
	if ($gamemode == Gameinfo::STNT) {
		$re_config['Challenge']['Current']['authortime']	= $aseco->server->challenge->gbx->ascore;		// AuthorScore
		$re_config['Challenge']['Current']['goldtime']		= $aseco->server->challenge->goldtime;
		$re_config['Challenge']['Current']['silvertime']	= $aseco->server->challenge->silvertime;
		$re_config['Challenge']['Current']['bronzetime']	= $aseco->server->challenge->bronzetime;
	}
	else {
		// All other GameModes
		$re_config['Challenge']['Current']['authortime']	= re_formatTime($aseco->server->challenge->authortime);	// AuthorTime
		$re_config['Challenge']['Current']['goldtime']		= re_formatTime($aseco->server->challenge->goldtime);
		$re_config['Challenge']['Current']['silvertime']	= re_formatTime($aseco->server->challenge->silvertime);
		$re_config['Challenge']['Current']['bronzetime']	= re_formatTime($aseco->server->challenge->bronzetime);
	}
	// TMX-Part
...
//		$xml .= '<quad posn="0 0 0" sizen="2.1 2.1" image="'. $re_config['IMAGES'][0]['WIDGET_OPEN_SMALL'][0] .'"/>';	// Loaded in Widgets, no need to preload
		...
		// Advertiser at Score
		$xml .= '<quad posn="0 0 0" sizen="3.87 4.03" image="http://labs.undef.de/.images/ingame/records-eyepiece/logo-records-eyepiece-normal.png" imagefocus="http://labs.undef.de/.images/ingame/records-eyepiece/logo-records-eyepiece-focus.png"/>';
...
function re_buildMusicWidget () {
	...
	if ($re_config['MUSIC_WIDGET'][0]['ADVERTISE'][0] == true) {
		$xml .= '<quad posn="9.5 -6.2 0.05" sizen="5.2 1.7" url="http://www.amazon.com/gp/search?ie=UTF8&amp;keywords='. rawurlencode(stripColors($re_config['CurrentMusicInfos']['Artist'], true)) .'&amp;tag=undefde-20&amp;index=digital-music&amp;linkCode=ur2&amp;camp=1789&amp;creative=9325" image="http://labs.undef.de/.images/ingame/records-eyepiece/logo-amazon-normal.png" imagefocus="http://labs.undef.de/.images/ingame/records-eyepiece/logo-amazon-focus.png"/>';
	}
...
function re_closeRaceDisplays ($login = false, $all = true) {
	...
	if ($all == false) {
		...
		$ids = array('07','08','09','10','11','12','13','14','30','31','32','33','35','37','38','44');
	}
			...
			foreach ($score as &$item) {
				if ($item['ladderrank'] >= $re_config['WINNING_PAYOUT'][0]['PLAYERS'][0]['RANK_LIMIT'][0]) {
					if ( ($re_cache['PlayerWinnings'][$item['login']]['FinishPayment'] + $re_cache['PlayerWinnings'][$item['login']]['FinishPaid']) < $re_config['WINNING_PAYOUT'][0]['PLAYERS'][0]['MAXIMUM_COPPERS'][0]) {
						if ( ($item['rights'] == true) && ($item['disconnected'] == false) ) {
							switch ($item['rank']) {
								case 1:
				...
				if ($gamemode == Gameinfo::RNDS) {
					...
					if ($re_config['LIVE_RANKINGS'][0]['GAMEMODE'][0][$gamemode][0]['DISPLAY_TYPE'][0] == true) {
						...
						if ( isset($re_config['CurrentGameInfos']['RoundsPointsLimit']) ) {
							$re_scores['LiveRankings'][$i]['score'] = $re_cache['CurrentRankings'][$i]['Score'] .'/'. $re_config['CurrentGameInfos']['RoundsPointsLimit'];
						}
						else {
...
function re_buildGerymaniaRecordsWidgetBody ($gamemode) {
	...
	// Calculate the widget height (+ 3.3 for title)
	$widget_height = ($re_config['LineHeight'] * $re_config['GERYMANIA_RECORDS'][0]['GAMEMODE'][0][$gamemode][0]['ENTRIES'][0] + 3.3);
	...
	// Add Background for top X Players
	if ($topcount > 0) {
		$build['header'] .= '<quad posn="0.4 -2.6 0.003" sizen="'. ($re_config['GERYMANIA_RECORDS'][0]['WIDTH'][0] - 0.8) .' '. (($topcount * $re_config['LineHeight']) + 0.3) .'" style="'. $re_config['STYLE'][0]['WIDGET_RACE'][0]['TOP_STYLE'][0] .'" substyle="'. $re_config['STYLE'][0]['WIDGET_RACE'][0]['TOP_SUBSTYLE'][0] .'"/>';
	}
...
function re_buildLiveRankingsWidget ($login, $preset, $gamemode, $limit = 50) {
	...
	if ( ($gamemode == Gameinfo::RNDS) && ($re_config['LIVE_RANKINGS'][0]['GAMEMODE'][0][$gamemode][0]['DISPLAY_TYPE'][0] == false) ) {
		...
		if ( isset($re_config['CurrentGameInfos']['RoundsPointsLimit']) ) {
			$placeholder = '0/'. $re_config['CurrentGameInfos']['RoundsPointsLimit'];
		}
		else {
			$placeholder = 0;
		}
...
function re_buildChallengeLastCurrentNextWindow () {
	global $re_config;
...
function re_buildMusiclistWindow ($page, $caller) {
	...
	if ($totalsongs > 0) {
		...
		for ($i = ($page * 20); $i < (($page * 20) + 20); $i ++) {
			...
			$xml .= '<quad posn="0.9 -6.9 0.05" sizen="5.2 1.7" url="http://www.amazon.com/gp/search?ie=UTF8&amp;keywords='. rawurlencode(stripColors($song['Artist'], true)) .'&amp;tag=undefde-20&amp;index=digital-music&amp;linkCode=ur2&amp;camp=1789&amp;creative=9325" image="http://labs.undef.de/.images/ingame/records-eyepiece/logo-amazon-normal.png" imagefocus="http://labs.undef.de/.images/ingame/records-eyepiece/logo-amazon-focus.png"/>';
			$xml .= '</frame>';
	...
	if ($re_config['CHALLENGE_WIDGET'][0]['ENABLED'][0] == true) {
		...
		if ($state == 'race') {
			...
			if ( isset($aseco->server->challenge->tmx->pageurl) ) {
				$xml .= $re_config['Templates']['CHALLENGE_DEFAULT']['TMXBUTTON'];
			}
...
function re_buildHelpWindow ($page) {
	...
	$xml = str_replace(
		...
		array(
			'BgRaceScore2',
			'LadderRank',
			'$L[http://labs.undef.de/XAseco1/Records-Eyepiece.php]Records-Eyepiece/'. $re_config['Version'] .'$L for XAseco',
			$buttons
		),
		$re_config['Templates']['WINDOW']['HEADER']
	);
	...
	$xml .= '<quad posn="56 0 0.11" sizen="18 46.5" image="http://labs.undef.de/.images/ingame/records-eyepiece/welcome-records-eyepiece-normal.jpg" imagefocus="http://labs.undef.de/.images/ingame/records-eyepiece/welcome-records-eyepiece-focus.jpg" url="http://labs.undef.de/XAseco1/Records-Eyepiece.php"/>';
	...
	if ($page == 0) {
		...
		$xml .= '<label posn="18 0 0.01" sizen="35 2" textsize="1" textcolor="FF0F" text="Display this help"/>';

		// Command "/eyepiece hide"
		$xml .= '<label posn="0 -2 0.01" sizen="17 2" textsize="1" textcolor="FFFF" text="/eyepiece hide"/>';
		$xml .= '<label posn="18 -2 0.01" sizen="35 2" textsize="1" textcolor="FF0F" text="Hide the Records-Widgets and store this as your preference"/>';

		// Command "/eyepiece show"
		$xml .= '<label posn="0 -4 0.01" sizen="17 2" textsize="1" textcolor="FFFF" text="/eyepiece show"/>';
		$xml .= '<label posn="18 -4 0.01" sizen="35 2" textsize="1" textcolor="FF0F" text="Show the Records-Widgets and store this as your preference"/>';

		// Command "/togglewidgets"
		$xml .= '<label posn="0 -9 0.01" sizen="17 2" textsize="1" textcolor="FFFF" text="/togglewidgets $FF0or$FFF F7"/>';
		$xml .= '<label posn="18 -9 0.01" sizen="35 2" textsize="1" textcolor="FF0F" text="Toggle the display of the Records-Widgets"/>';

		// Command "/estat [PARAMETER]"
		$xml .= '<label posn="0 -12 0.01" sizen="17 2" textsize="1" textcolor="FFFF" text="/estat [PARAMETER]"/>';
		$xml .= '<label posn="18 -12 0.01" sizen="35 2" textsize="1" textcolor="FF0F" text="Optional parameter can be:$FFF'. LF .'dedirecs, geryrecs$FF0*$FFF, localrecs, topnations, topranks, topwinners,'. LF .'mostrecords, mostfinished, topplaytime, topdonators, toptracks,'. LF .'topvoters, topvisitors, topactive, toppayouts, topbetwins$FF0*'. LF .'*) Only if the related Plugin is enabled"/>';

		// Command "/emusic"
		if ($re_config['MUSIC_WIDGET'][0]['ENABLED'][0] == true) {
			$xml .= '<label posn="0 -23 0.01" sizen="17 2" textsize="1" textcolor="FFFF" text="/emusic"/>';
			$xml .= '<label posn="18 -23 0.01" sizen="35 2" textsize="1" textcolor="FF0F" text="Lists musics currently on the server"/>';
		}

		// Command "/elist [PARAMETER]"
		$xml .= '<label posn="0 -26 0.01" sizen="17 2" textsize="1" textcolor="FFFF" text="/elist [PARAMETER]"/>';
		$xml .= '<label posn="18 -26 0.01" sizen="35 2" textsize="1" textcolor="FF0F" text="Lists tracks currently on the server, optional parameter can be:'. LF .'$FFFjukebox, author, track, norecent, onlyrecent, norank, onlyrank,'. LF .'nomulti, onlymulti, noauthor, nogold, nosilver, nobronze, nofinish,'. LF .'$FFFbest, worst, shortest, longest, newest, oldest, bestkarma,'. LF .'worstkarma $FF0or a keyword to search for"/>';
...
function re_buildWelcomeWindow ($send = false, $login, $nickname) {
	...
	$re_config['WELCOME_WINDOW'][0]['MESSAGE'][0] = str_replace('{br}', LF, $re_config['WELCOME_WINDOW'][0]['MESSAGE'][0]);
	$re_config['WELCOME_WINDOW'][0]['MESSAGE'][0] = str_replace('{server}', re_handleSpecialChars(stripColors($aseco->server->name)), $re_config['WELCOME_WINDOW'][0]['MESSAGE'][0]);
	$re_config['WELCOME_WINDOW'][0]['MESSAGE'][0] = str_replace('{player}', re_handleSpecialChars($nickname.'$Z'), $re_config['WELCOME_WINDOW'][0]['MESSAGE'][0]);
	...
	if ($re_config['WELCOME_WINDOW'][0]['IMAGE'][0]['NORMAL'][0] != '') {
		...
		$xml .= '/>';
		$xml .= '<label posn="3 -6 0.01" sizen="54 0" autonewline="1" textsize="1" textcolor="FF0F" text="'. $re_config['WELCOME_WINDOW'][0]['MESSAGE'][0] .'"/>';
	}
	else {
		$xml .= '<label posn="3 -6 0.01" sizen="72.4 0" autonewline="1" textsize="1" textcolor="FF0F" text="'. $re_config['WELCOME_WINDOW'][0]['MESSAGE'][0] .'"/>';
	}
...
function re_loadTemplates () {
	...
	$header .= '<label posn="2.3 -4.9 0.1" sizen="6.35 2" halign="center" textcolor="'. $re_config['LADDERLIMIT_WIDGET'][0]['TEXT_COLOR'][0] .'" scale="0.6" text="DONATE"/>';
	...
	$tmx = '<quad posn="14.4 -6.05 0.05" sizen="4.4 2.2" halign="right" action="'. $re_config['ManialinkId'] .'08" image="'. $re_config['IMAGES'][0]['TMX_LOGO_NORMAL'][0] .'" imagefocus="'. $re_config['IMAGES'][0]['TMX_LOGO_FOCUS'][0] .'"/>';
	...
	$templates['CHALLENGE_DEFAULT']['TMXBUTTON'] = $tmx;
	...
	$content .= '<label posn="2.3 -4.9 0.1" sizen="6.35 2" halign="center" textcolor="'. $re_config['TRACKCOUNT_WIDGET'][0]['TEXT_COLOR'][0] .'" scale="0.6" text="FAVORITE"/>';
	$content .= '</frame>';
	$content .= '</manialink>';
	...
	$race .= '<quad posn="'. $re_config['EYEPIECE_WIDGET'][0]['RACE'][0]['POS_X'][0] .' '. $re_config['EYEPIECE_WIDGET'][0]['RACE'][0]['POS_Y'][0] .' 0" sizen="6.19 6.45" url="http://labs.undef.de/XAseco1/Records-Eyepiece.php" image="http://labs.undef.de/.images/ingame/records-eyepiece/logo-records-eyepiece-opacity.png" imagefocus="http://labs.undef.de/.images/ingame/records-eyepiece/logo-records-eyepiece-focus.png"/>';
	$race .= '</manialink>';
	...
	$score .= '<quad posn="0 0 0.001" sizen="4.6 6.5" url="http://labs.undef.de/XAseco1/Records-Eyepiece.php" style="'. $re_config['EYEPIECE_WIDGET'][0]['SCORE'][0]['BACKGROUND_STYLE'][0] .'" substyle="'. $re_config['EYEPIECE_WIDGET'][0]['SCORE'][0]['BACKGROUND_SUBSTYLE'][0] .'"/>';
	$score .= '<quad posn="-0.18 -4.6 0.002" sizen="2.1 2.1" image="'. $re_config['IMAGES'][0]['WIDGET_OPEN_SMALL'][0] .'"/>';
	$score .= '<quad posn="0.365 -0.3 0.002" sizen="3.87 4.03" url="http://labs.undef.de/XAseco1/Records-Eyepiece.php" image="http://labs.undef.de/.images/ingame/records-eyepiece/logo-records-eyepiece-normal.png" imagefocus="http://labs.undef.de/.images/ingame/records-eyepiece/logo-records-eyepiece-focus.png"/>';
...
function re_getTracklist ($trackfile = false) {
	...
	if ($trackfile == false) {
		...
		// Parse the GBX Challenge file
		$gbx = new GBXChallengeFetcher($aseco->server->trackdir . $trackfile, true, false);
		if (!$gbx->uid) {
			// Ignore if Track could not be parsed
			trigger_error('[plugin.records_eyepiece.php] Could not read Track ['. $aseco->server->trackdir . $trackfile .'] at re_getTracklist()', E_USER_WARNING);
		}
		else {
	...
	if ( !empty($trackinfos) ) {
		...
		if (count($re_cache['Tracklist']) > 0) {
			...
			if (function_exists('mb_substr')) {
				foreach ($re_cache['Tracklist'] as &$track) {
					if ( mb_substr($track['name_plain'], 0, 3) == pack("CCC", 0xef, 0xbb, 0xbf) ) {
						$track['name_plain'] = mb_substr($track['name_plain'], 3);
					}
				}
				unset($track);
			}
...
function re_getMapInfoGBX ($filename) {
	...
	// Parse the GBX Challenge file
	$gbx = new GBXChallengeFetcher($aseco->server->trackdir . $filename, true, false);
	if (!$gbx->uid) {
		trigger_error('[plugin.records_eyepiece.php] Could not read Track ['. $aseco->server->trackdir . $filename .'] at re_getMapInfoGBX()', E_USER_WARNING);
	...
	if ($aseco->server->gameinfo->mode == Gameinfo::STNT) {
		$track['authortime']		= $gbx->ascore;				// AuthorScore
		$track['goldtime']		= $gbx->goldtm;
		$track['silvertime']		= $gbx->silvertm;
		$track['bronzetime']		= $gbx->bronzetm;

		// Unformated for Tracklist-Filter
		$track['authortime_filter']	= $gbx->ascore;				// AuthorScore
		$track['goldtime_filter']	= $gbx->goldtm;
		$track['silvertime_filter']	= $gbx->silvertm;
		$track['bronzetime_filter']	= $gbx->bronzetm;
	}
	else {
		// All other GameModes
		$track['authortime']		= re_formatTime($gbx->authortm);	// AuthorTime
		$track['goldtime']		= re_formatTime($gbx->goldtm);
		$track['silvertime']		= re_formatTime($gbx->silvertm);
		$track['bronzetime']		= re_formatTime($gbx->bronzetm);

		// Unformated for Tracklist-Filter
		$track['authortime_filter']	= $gbx->authortm;			// AuthorTime
		$track['goldtime_filter']	= $gbx->goldtm;
		$track['silvertime_filter']	= $gbx->silvertm;
		$track['bronzetime_filter']	= $gbx->bronzetm;
	}
